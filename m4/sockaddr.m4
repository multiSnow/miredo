...
# SHOULD check <sys/socket.h>, <winsock2.h> before that
...
# SHOULD check <winsock2.h> before that
AC_DEFUN([RDC_FUNC_SOCKET],
[AC_LANG_ASSERT(C)
AC_SEARCH_LIBS(socket, [socket], rdc_cv_func_socket=yes,
# AC_SEARCH_LIBS(socket, [ws2_32]) does not work with Mingw32
...
# SHOULD check <sys/socket.h>, <netdb.h>, <winsock2.h> before that
# NOTE: does not check required library
# (-lresolv is NOT needed on Solaris, but -lws2_32 is required on Windows)
...
# Cannot have getnameinfo if not getaddrinfo
...
# Cannot have gai_strerror if not getaddrinfo
